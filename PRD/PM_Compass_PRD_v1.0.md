...
PM Compass uses a two-layer evaluation framework: retrieval evals (offline, deterministic) and LLM-as-judge evals (generative, scalable). Both layers are required — neither alone is sufficient to assess system health.

```
Question → [RETRIEVAL] → Chunks → [GENERATION] → Answer
               ↑                        ↑
         Precision/Recall          LLM-as-Judge
         (retrieval quality)       (answer quality)
```

### Layer 1 — Retrieval Evaluation (Offline)

**Method:** Golden dataset of 67 QA pairs auto-generated by GPT-4o-mini from source chunks (regenerated after Drive corpus migration). Each pair contains a question, expected chunk, and source file.
...
### Layer 2 — LLM-as-Judge Evaluation (Generative)
